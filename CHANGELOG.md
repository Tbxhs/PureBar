...
## [2.6.0] - 2026-08-16
- 新增"已完成的提醒"显示样式设置：删除线、变淡显示或完全隐藏，可在"偏好设置 › 日历"中配置
- 设置项收缩到"偏好设置"子菜单，右键菜单简化为：偏好设置 / 关于与帮助 / 退出
- 日历下方的事件列表与详情弹窗对已完成任务应用统一的视觉样式
- 清理改名残留：移除引用旧 LunarBar.xcodeproj 的陈旧 scheme，新增含测试动作的 PureBarMac scheme
- 修复测试宿主 TEST_HOST 指向错误可执行文件（LunarBar → PureBar）导致测试无法运行的问题
- 更新 HolidayManagerTests 到当前缓存文件 API，移除孤立测试文件

## [2.5.3] - 2026-04-16
- 优化月份切换动画，仅滑动变化的部分（年/月独立动画）
- 采用 ticker 风格过渡效果，跨年切换带微妙时间差

## [2.5.2] - 2026-04-16
- 点击历史日期可查看当天完整日程
- 优化弹窗打开和"回到今天"时的事件加载流程，消除闪烁

## [2.5.1] - 2026-02-09
- 修复 modernStyle 模式下"今"字图标尺寸不匹配的 bug
- 修复 textBadge 图标在约束中被压缩的问题
- 节假日图标风格菜单项增加图标预览
- 获取假日数据后显示成功/失败反馈提示

## [2.5.0] - 2026-02-09
- textBadge 模式下"回到今天"按钮显示"今"字图标，保持文字风格一致
